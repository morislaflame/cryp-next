...
export interface CardNavProps {
  logo: string | { src: string; width: number; height: number; blurWidth?: number; blurHeight?: number; blurDataURL?: string };
  logoAlt?: string;
  items: CardNavItem[];
  className?: string;
  ease?: string;
...
const CardNavStatic: React.FC<CardNavProps> = ({
  logo,
  logoAlt = 'Logo',
  items,
  className = '',
      ...
      >
        <div className="absolute top-0 left-0 right-0 h-[60px] flex items-center justify-between px-[1.1rem] z-[2] md:px-[1.1rem] px-4">
          <Link href={MAIN_ROUTE} className="flex items-center cursor-pointer">
            <img src={typeof logo === 'string' ? logo : logo.src} alt={logoAlt} className="h-7" />
          </Link>
